...
struct Vector2 {
	...
	T Dot(const Vector2& v) const//点乘
	{
		return x * v.x + y * v.y;
	}
    ...
    Vector2 Normalized() const//标准化输出向量
    {
        Vector2 v = *this;
        v.Normalize();
        return v;
    ...
    operator Vector2<float>() const// Vector<*>  ==>  Vector<float>  (类型转换)
    {
    ...
    Vector2 Lerp(float t, const Vector2& v) const	//线性插值
    {
        return Vector2(x * (1 - t) + v.x * t,
                       y * (1 - t) + v.y * t);
    }
    template <typename P>	//保存到内存
    P* Write(P* pData)
    {
